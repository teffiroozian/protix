...
export default function StickyRestaurantBar({
  ...
  const [isControlsDocked, setIsControlsDocked] = useState(() => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsControlsDocked(!entry.isIntersecting);
      },
      { threshold: 0 }
    );
  ...
  return (
    <div className="fixed left-0 right-0 top-0 z-50 shadow-[0_6px_16px_rgba(15,23,42,0.14)] transition-shadow duration-300">
      ...
      {hasActiveFilters && isControlsDocked ? (
        <div className="w-full border-b border-slate-200/70 bg-white/95 backdrop-blur transition-all duration-300">
