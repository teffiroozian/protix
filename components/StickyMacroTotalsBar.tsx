...
export default function StickyMacroTotalsBar({
  ...
  const panelClassName = inline
    ? "w-full rounded-3xl border border-black/10 bg-[#f5f5f5] px-4 py-4 shadow-sm sm:px-5 sm:py-5"
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:gap-8">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto">
            <button
              type="button"
              onClick={onSaveMeal}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-black/80 bg-transparent px-6 text-base font-semibold text-[#1A1A1A] transition hover:bg-black/5"
            >
              <Bookmark className="h-4 w-4" strokeWidth={2.25} aria-hidden="true" />
              <span>Save Meal</span>
            </button>
            <button
              type="button"
