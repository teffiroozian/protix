...
export default function CartPreviewDrawer() {
  ...
  const activeRestaurant = useMemo(() => {
    ...
    return restaurants.find((restaurant) => restaurant.id === activeRestaurantId) ?? null;
  }, [items]);
        ...
        <div className="flex h-full flex-col">
          ...
          <section className="flex-1 overflow-y-auto px-5 py-4">
            ...
            {items.length === 0 ? (
              ...
              <ul className="mt-3 space-y-3 pb-2">
                {items.map((item) => (
                  <li key={item.id} className="rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-sm">
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-medium uppercase tracking-wide text-slate-600">
                          ...
                          <span>{formatMacro("g protein", item.macrosPerItem.protein)}</span>
          ...
          <section className="shrink-0 border-t border-slate-200 px-5 py-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Total Macros</p>
              <div className="mt-3 grid grid-cols-4 gap-2 text-center">
                <div className="rounded-xl border border-slate-200 bg-white px-2 py-2">
                  <p className="text-[11px] uppercase tracking-wide text-slate-500">Calories</p>
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{totals.calories}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-2 py-2">
                  <p className="text-[11px] uppercase tracking-wide text-slate-500">Protein</p>
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{totals.protein}g</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-2 py-2">
                  <p className="text-[11px] uppercase tracking-wide text-slate-500">Carbs</p>
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{totals.carbs}g</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-2 py-2">
                  <p className="text-[11px] uppercase tracking-wide text-slate-500">Fat</p>
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{totals.fat}g</p>
                </div>
              </div>
            </div>
