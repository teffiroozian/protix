...
export default function RestaurantView({
  ...
  if (isChipotleBuildPage && isEditingBuild) {
    return (
      <div className="fixed inset-0 z-[130] flex items-end justify-center sm:items-center" role="dialog" aria-modal="true" aria-label={`${editingCartItem.name} customization`}>
        <button
          type="button"
        ...
        />
        <div className="relative flex h-[min(100dvh,100%)] w-full flex-col overflow-hidden rounded-t-2xl bg-white px-3 pt-3 sm:m-4 sm:h-[calc(100%-32px)] sm:w-[min(1024px,calc(100%-32px))] sm:rounded-2xl sm:px-6 sm:pt-6">
          <button
            type="button"
          ...
          <div ref={buildCustomizationModalScrollRef} className="min-h-0 flex-1 overflow-y-auto pb-8 pr-1 [overflow-anchor:none] sm:pb-10 sm:pr-2">
            <div className="grid justify-items-center gap-8">
              <div className="grid justify-items-center gap-5">
                <h1 className="text-center text-[28px] font-extrabold leading-tight sm:text-[32px]">{editingCartItem.name}</h1>
                <img
                  className="h-[240px] w-full max-w-[300px] rounded-[14px] bg-[#efefef] object-contain p-2 shadow-[0_0_5px_rgba(0,0,0,0.25)] sm:h-[300px]"
          ...
          <div className="-mx-3 z-10 flex flex-wrap items-center gap-3 border-t border-black/10 bg-white p-3 shadow-[0_-4px_10px_rgba(0,0,0,0.08)] sm:-mx-6 sm:justify-between sm:p-4">
            <MacroTotalsGrid
              macros={{
                calories: Math.round(adjustedSelectedIngredientTotals.calories ?? 0),
                protein: Math.round(adjustedSelectedIngredientTotals.protein ?? 0),
